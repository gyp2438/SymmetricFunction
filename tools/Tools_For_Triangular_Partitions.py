...
def A_v(v,Formal=True):
    n=len(v)
    rep=add(gamma_mu_v(mu,v)*T_mu(mu) for mu in Partitions(n))
    if Formal:
        return InSchur(rep)
    else:
        return rep
...
def deg_mu(tau,mu):
    return add(tau)-add(binomial(k,2) for k in mu)
...
def eta(mu):
    mu=Partition(mu)
    return add(j for (i,j) in mu.cells())
...
def E_v(v,Formal=False):
    n=len(v)
    rep=s(add(gamma_mu_v(mu,v)*H(mu.conjugate())
                                  for mu in Partitions(n)))
    if Formal:
        return Formal_Coefficients(rep)
    else:
        return rep

def E_vt(v,Formal=False):
    n=len(v)
    rep=s(add(Weight_t(mu,v)*Ht(mu.conjugate()) for mu in Partitions(n)))
    if Formal:
        return Formal_Coefficients(rep)
    else:
        return rep
...
def flip(g):
    rep=g.map_coefficients(lambda c:c.substitute({q:1/q}))
    d=max([(c.denominator()).degree() for (mu,c) in rep])
    return q**d*rep


def Formal_Symmetric(F):
    return add(tensor([InSchur(c)]+[s(mu) for mu in v]) for v,c in F.To_b())
...
@cached_function
def gamma_mu_v(mu,v):
    return add(w_theta_v(theta,v)*Omega(theta) for theta in StandardTableaux(mu))
...
@cached_function
def lambda_mu_v(v,mu):
    mu=Partition(mu)
    return gamma_mu_v(mu.conjugate(),v)/gamma_mu_v(mu.conjugate(),tuple([0 for i in range(len(v))]))
...
@cached_function
def min_concave(tau):
    mu=Partition(list(tau))
    if mu.length()==0:
        return tuple([0])
    else:
        m=min([2*len(tau)]+[i for i in range(len(tau)+1,2*len(tau)+Integer(1)) 
							if Is_Concave(mu.Plus(tuple(1 for j in range(i))))])-len(tau)
        return tuple(tau)+tuple(0 for j in range(m))
...
def Nabla_v(v,g):
    v=tuple(tuple([0])+v[1:])
    return s(add(c*lambda_mu_v(v,mu)*H(mu) for (mu,c) in H(g)))

def Nabla_tv(v,g):
    return Nabla_v(v,g).map_coefficients(lambda c:c.substitute({t:1}))
...
def Omega(theta):
    mu=theta.shape()
    C=mu.cells()
    return (mul((star(z(c)-z(d))*star(z(c)-q*t*z(d))
                 /(star(z(c)-q*z(d))*star(z(c)-t*z(d)))) 
                for d in C for c in C if theta.entry(d)<theta.entry(c))
            *mul(z(c)/(z(c)-q*t*z(d))
                 for d in C for c in C if theta.entry(d)+1==theta.entry(c))
            *mul(1/(z(d)-1)
                 for d in C if not d==(0,0)))

def omegastar(f):
    return f.map_coefficients(lambda c:c.substitute({t:1/t,q:1/q})).omega()
...
def part_to_vect(mu,n=0):
    mu=Partition(mu)
    if mu.length()==0:
        return tuple([infinity]+[0 for i in range(n-1)])
    else:
        if n==0: n=mu.length()+1
        L=list(mu)+[0]
        DL=[L[i]-L[i+1] for i in range(len(L)-1)]
        return tuple([infinity]+DL+[0 for i in range(n-len(DL)-1)])
...
def poids_tamari(alpha,beta,Tammu,n):
    c=Tammu.interval(alpha,beta)
    return (q**(max([len(mm) for mm in Tammu.subposet(c).maximal_chains()])-1)*
            s(to_part(beta).Plus(Partition([n]).conjugate())).skew_by(s(to_part(beta))))
...
def SpringDrawPoset(P,epais=1,Sommets=False):
    return P.cover_relations_graph().to_undirected().show(method="js",
                                                          vertex_size=3,
                                                          vertex_labels=Sommets,
                                                          edge_thickness=epais,
                                                          link_distance=50)
...
from sage.combinat.nu_tamari_lattice import NuTamariLattice

@cached_function
def Tamari(mu):
    mu=Partition(mu)
    return NuTamariLattice(mu.to_dyck_word())
...
def Tamari_q_z(mu,n):
    ...
    if n==None:
        n=len(mu)+1
    return e(add(poids_tamari(alpha,beta,Tammu,n) 
               for alpha in Tammu 
               for beta in Tammu 
               if not Tammu.interval(alpha,beta)==[]))

@cached_function
def Tam_q_Enumer(mu,n=None):
    if n==None:
        n=len(mu)+1
    return Tamari_q_z(mu,n).scalar(e[n])


def Theta(g,f):
    return Pi_op(g(Z/M)*Pi_op_inv(f))
...
def to_part(beta):
    w=beta.height()
    bp=[(c[0],w-c[1]) for c in beta.points()]
    return Partition([min([c[0] for c in bp if c[1]==i]) for i in range(w)])
...
@cached_function
def vect_to_part(v):
    return Partition(reversed(Composition(reversed(v[1:])).partial_sums()))
	
def w(mu):
    mu=Partition(mu)
    return add((i+1)*mu[i] for i in range(mu.length()))

def w_theta_v(theta,v):
    mu=theta.shape()
    return mul((q**i*t**j)**(v[theta.entry((i,j))-1]-1) for i,j in mu.cells() if not (i,j)==(0,0))

def Weight_t(mu,v):
    return add(q**wt(theta,v)*Omega(theta).substitute({t:1}) for theta in StandardTableaux(mu))

def wt(tab,v):
    mu=tab.shape()
    return add(c[0]*(v[tab.entry(c)-1]-1) for c in mu.cells())
...
def DiagCellLines(tau):
    return sorted([c[1]-1 for c in Diagonale(tau)])

def Gauche(tau):
    tau=Partition(tau)
    if not Is_Dominant(tau):
        return Set([mu.conjugate() for mu in Droite(tau.conjugate())])
    else:
        L=DiagCellLines(tau)
        return Set([RemoveCells(tau,L[:i+1]) for i in range(len(L))])

def Droite(tau):
    tau=Partition(tau)
    if not Is_Dominant(tau):
        return Set([mu.conjugate() for mu in Gauche(tau.conjugate())])
    else:
        L=DiagCellLines(tau)
        return Set([RemoveCells(tau,L[i:]) for i in range(len(L))])
...
def Face(tau):
	...
	if tau.length()==0:
		...
		return sorted(TriangularPredecessors(tau).union(Set([tau])))
	else:
		return sorted(Set([tau]).union(Gauche(tau).union(Droite(tau))))

def RemoveCells(tau,L):
    mu=Partition(tau)
    if len(L)==0:
        return tau
    else:
        return RemoveCells(tau.remove_cell(L[0]),L[1:])
