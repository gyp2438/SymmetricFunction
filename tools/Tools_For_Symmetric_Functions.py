...
def MacCombScalar(mu):
    mu=Partition(mu)
    return (-1)**(mu.size()-mu.length())*p(mu)(M)*mu.aut()

def WhittScalar(mu):
    mu=Partition(mu)
    return (p(mu)((1-q)))*mu.aut()
...
Ht=Sym.macdonald(t=1).Ht()
Ht.print_options(prefix="H",latex_prefix="H")
Ht._latex_term = lambda mu: "\\boldsymbol{1}" if mu==zero else "\\overline{H}_{%s}"%(''.join(mystr(i) for i in mu))
...
def Is_Positive(F):
    if F==0:
        ...
        return min([0]+[min(c.numerator().coefficients()) for (mu,c) in F])>= 0
