...
@add_method(ClassePartages)
def nabla(self,power=1):
    n=self.length()
    rep=[self[j]+power*(n-j) for j in range(n)]
    if Min(rep)>=0:
        return Partition([self[j]+power*(n-j) for j in range(n)])
    else:
        return None
    
@add_method(ClassePartages)
def conabla(self,power=1):
    return self.conjugate().nabla(power=power).conjugate()
