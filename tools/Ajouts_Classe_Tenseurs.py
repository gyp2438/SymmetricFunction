...
def Print_tensor(F,tau): 
    Liste=Set([nu for (mu,nu),c in F.To_b(base=s)])
    print("Partition(",tau,"):(")
...
def CoBar(nu):
    return Bar(nu.conjugate()).conjugate()
...
@add_method(ClasseTenseurs)
def cobar(self,composante=-1,base=s):
    self=(self).To_b(base)
    return add(c*tensor([s(mu) for mu in v[:-1]]+[base(CoBar(v[-1]))]) for v,c in self)
...
@add_method(ClasseTenseurs)
def Dim(self):
    return self.scalar(e[1]**self.degree())
...
@add_method(ClasseTenseurs)
def map_fonction(self,fonction,composante=0,g=s):
    self=self.To_b(g)
    if composante==1:
        return add(c*tensor([s(mu),fonction(g(nu))]) for (mu,nu),c in (self).To_b(g))
    elif composante==0:
        gdual=g.dual_basis()
        Liste=sorted(Set([v[-1] for v,c in self.To_b(g)]))
        return add(tensor([fonction(self.scalar(gdual(mu))),g(mu)]) for mu in Liste)
    else:
        self

@add_method(ClasseTenseurs)
def map_coefficient(self,fonction,g=e,tenseur=False):
    self=self.To_b(g)
    gdual=g.dual_basis()
    Liste=sorted(Set([v[-1] for v,c in self]))
    if tenseur:
        return add(tensor([fonction(self.scalar(gdual(mu))),g(mu)]) for mu in Liste)
    else:
        return add(fonction(self.scalar(gdual(mu)))*g(mu) for mu in Liste)
...
@add_method(ClasseTenseurs)
def omega(self,k=0,base=s):
    return add(c*tensor([s(mu) for mu in v[:k]] 
                        + [s(v[k]).omega()] 
                        + [s(mu) for mu in v[k+1:-1]]
                        + [base(v[-1])]) for v,c in self.To_b(base))
...
@add_method(ClasseTenseurs)
def oplus(self,B,base=s):
    return self+add(c*tensor([s(nu) for nu in v]) for v,c in (B-self).To_b(base) if c>0)


@add_method(ClasseTenseurs)
def PP(self,base=s,f_coeff=s):
    if self==0*tensor([s[0],s[0]]):
        return 0
    else:
        Liste=Set([v[-1] for v,c in self.To_b(base)])
        return Dishout(self,g=base,Famille=Liste,f_coeff=f_coeff)
...
def Dishout(F,g=None,Famille=None,f_coeff=s):
    if F==0: 
        show(0)
    elif g==None:
        ...
        if n==0:
            ...
            if Famille==None:
                Liste=sorted(Set([v[-1] for v,c in F.To_b(g)]))
            else:
                Liste=sorted(Famille)
            for mu in Liste:
                First=Liste[0]
                gdual=g.dual_basis()
                val=f_coeff(F.scalar(gdual(mu)))
...
@add_method(ClasseTenseurs)
def restrict_length(self,k,exact=False,base=s):
    if exact:
        return add(c*tensor([s(v[0]),base(v[1])]) for v,c in self.To_b(base) 
                   if v[0].length()==k)
    else:    
        return add(c*tensor([s(v[0]),base(v[1])]) for v,c in self.To_b(base) 
                   if v[0].length()<=k)
...
@add_method(ClasseTenseurs)
def Eval(self,g,base=s,composante=0,exclus=None):
    if self==0:
        return self
    else:
        k=composante
        rep=add(c*s(v[k])(g*Un,exclude=exclus).scalar(Un)*tensor([s(mu) for mu in v[:k]] 
                        + [s(mu) for mu in v[k+1:-1]]+ [base(v[-1])]) 
               for v,c in self.To_b(base))
        if rep.length()==1:
            return add(c*base(v[0]) for v,c in rep)
        else:
            return rep
...
@add_method(ClasseTenseurs)
def To_b(self,base=s):
    if base==e:
        return tensor([s for i in range(self.length()-1)]+[e])(self)
    else:
        return tensor([s for i in range(self.length())])(self)

@add_method(ClasseTenseurs)
def Up(self,k,base=e):
    return add(c*tensor([(s(mu)*e[k]).restrict_partition_lengths(k),base(nu)]) for (mu,nu),c in self.To_b(base))
